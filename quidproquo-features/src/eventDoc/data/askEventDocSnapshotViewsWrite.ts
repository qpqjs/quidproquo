 ...
 */
export function* askEventDocSnapshotViewsWrite(
  docId: string,
  eventId: number,
  snapshotViews: EventDocSnapshotViews,
  ...
  for (const viewName of viewNames.filter((name) => name !== EVENT_DOC_PRIMARY_VIEW)) {
    yield* askEventDocSnapshotWrite(docId, viewName, eventId, snapshotViews[viewName], snapshotCacheKey);
  }

  yield* askEventDocSnapshotWrite(docId, EVENT_DOC_PRIMARY_VIEW, eventId, snapshotViews[EVENT_DOC_PRIMARY_VIEW], snapshotCacheKey, viewNames);
}
