...
export function* eventDocAiChatContinue(event: ExecuteServiceFunctionEvent<EventDocAiChatContinuePayload>): AskResponse<void> {
  const { docId, chatId } = event.payload;
  ...
  const result = yield* askCatch(askEventDocAiContextProvide({ serviceName, type, docId }, askEventDocAiProcessContinue(docId, chatId)));
