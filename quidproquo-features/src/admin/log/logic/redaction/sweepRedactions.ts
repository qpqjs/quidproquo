...
/**
 * Replaces every occurrence of each value, as a substring, in every string leaf of the tree.
 * Longest values go first so a secret that contains another leaves no fragment behind.
 */
export const sweepRedactions = <T>(value: T, redactions: string[]): T => {
  ...
  return mapStrings(value, (text) => text.replace(pattern, REDACTED_STRING));
};
