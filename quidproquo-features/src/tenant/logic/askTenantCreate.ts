...
// Create the tenant eventDoc and link the creator as its first member (both
// membership directions). Must run under the tenant eventDoc store context (the
// tenant routes provide it).
export function* askTenantCreate(name: string, actor: EventDocEventActor): AskResponse<EventDocSummary> {
  ...
  yield* askTenantLinkMember(summary.id, actor.userId);
