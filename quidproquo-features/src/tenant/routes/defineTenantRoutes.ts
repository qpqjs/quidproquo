...
// The membership-gated tenant routes (list mine / create / get-record / get-logo / my
// membership / list, add, update and remove members / set roles / the role catalog),
...
export const defineTenantRoutes = ({ myTenantsBasePath, routeAuthSettings, version, tenantHeaderName }: TenantRoutesOptions): QPQConfig => {
  ...
  return [
    ...
    route('GET', `${myTenantsBasePath}/{id}/membership`, 'getMembership'),
    route('GET', `${myTenantsBasePath}/{id}/roles`, 'listRoles'),
    ...
    route('PUT', `${myTenantsBasePath}/{id}/members/{userId}/roles`, 'setMemberRoles'),
  ];
};
