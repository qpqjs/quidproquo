...
```
apps/<app>/                       # qpqjs
  deploy.config.json              # { "deployments": { "<name>": { platform, name, environment, ... } } }
  devServer.config.ts             # optional: dev server ports (also the docker image's) + local QPQ config overrides
...
`qpq go` needs AWS credentials plus a deployment: an entry in `apps/<app>/deploy.config.json` selected with `--deployment <name>`. Each entry is the whole story for that target (`platform`, `name`, `environment`, optional `feature`, `platformSettings` such as `accountId`/`region` for aws, and optional string `settings` that reach `infrastructure.ts` through `getDeploySetting`). The file is the source of truth: no env var or flag overrides it, so to change a target you edit the file or add an entry. Service configs read the primed `APPLICATION_NAME`, `ENVIRONMENT` and `FEATURE_NAME` env vars. App-specific bootstrap and account extras live in `apps/<app>/bootstrap.qpq.ts` and `apps/<app>/account.qpq.ts` as config fragments; the identity plumbing is provided by the generic CDK app in `quidproquo-deploy-awscdk`.
