...
- `FileStorageConfig` (`quidproquo-actionprocessor-node`) gains a required `secureUrlPublicPort: number`, the port secure file urls are generated with; code constructing one by hand (tests, custom dev-server setups) must add it, typically the same value as `secureUrlPort`. `DevServerConfigOverrides.fileStorageConfig`'s equivalent `secureUrlPublicPort` stays optional and defaults to `secureUrlPort`. An app can now set its dev server's ports via a new optional `ports: { api?, webSocket?, fileStorage? }` on `apps/<app>/devServer.config.ts`'s default export, overriding the 8080/8888/3001 defaults everywhere (local run, docker image, generated urls) instead of only in the generated entry.
- Docker deployments no longer require the file storage port mapping to be identity (`host === container`); the host port is instead baked into the image as `QPQ_PUBLIC_FILE_STORAGE_PORT` so secure urls carry the real externally-reachable port. `writeComposeFile` (`quidproquo-cli`) now requires a `publicFileStoragePort: number` option.
- `quidproquo-cli`'s docker helpers change shape for anything importing them directly: `DEV_SERVER_PORTS` (`lib/devServerPorts`) is replaced by `DEFAULT_DEV_SERVER_PORTS` plus `readDevServerPorts(appName)` (resolves the app's `devServer.config.ts` overrides); `BASE_CONTAINER_PORTS` is removed and `getContainerPorts(hosts)` becomes `getContainerPorts(devServerPorts, hosts)`; `getDefaultPortMappings(containerPorts)` becomes `getDefaultPortMappings(apiPort, containerPorts)`; `resolvePortMappings(deploymentName, settings, containerPorts)` becomes `resolvePortMappings(deploymentName, settings, apiPort, containerPorts)`.

- `apps/<app>/deploy.config.json` changes shape: `{ "prefix", "environments": { "<env>": { platform?, accountId?, region? } } }` becomes `{ "deployments": { "<name>": { "platform", "name", "environment", "feature"?, "platformSettings"?, "settings"? } } }`. `name` (the old `prefix`) and `environment` move onto each entry, aws identity moves under `platformSettings` (`accountId`, `region`), and an optional `settings` map of string values is delivered to `infrastructure.ts` as `DEPLOY_SETTING_<KEY>` env vars (read with the new `getDeploySetting` / `getDeploySettingOrDefault` from `quidproquo-core`). The file is now the source of truth: env vars and flags never override it, so a feature deployment is an entry with `feature` set rather than an `ACTOR_NAME` in the shell.
- `qpq` commands take `--deployment <name>` instead of `--env <name>`; `--platform` is removed (the platform comes from the entry). This applies to `go`, `go:docker`, `go:dev`, `go:dev:api`, `go:dev:web`, `synth`, `migrate`, `setup`, `publish`, `teardown` and `clear-resources`. With several entries and no selection, a headless run now exits instead of falling back to `ENVIRONMENT`. CI workflows passing `--env` must change.
- The env vars primed for service configs are `APPLICATION_NAME`, `ENVIRONMENT`, `FEATURE_NAME` (was `ACTOR_NAME`), `DEPLOY_NAME`, `DEPLOY_APP_NAME` and, for aws, `AWS_DEFAULT_ACCOUNT` / `AWS_DEFAULT_REGION`. Service-utils helpers should read `process.env.APPLICATION_NAME` for the application name instead of hardcoding it, and `process.env.FEATURE_NAME` for the feature.
- `quidproquo-deploy-awscdk`: `QpqAppDeployConfig`, `QpqAppDeployEnvironment` and `getQpqAppDeployConfig` are removed from this package. `getQpqAppDeployContext(root, appName, deploymentName)` replaces the `(root, appName, environment, actorName, envOverride)` signature; `QpqAppDeployContext` fields `prefix` and `actorName` become `name` and `feature`, and it gains `deploymentName` and `settings`. The generic CDK app requires `DEPLOY_NAME` alongside `DEPLOY_APP_NAME` and no longer reads `ENVIRONMENT`, `ACTOR_NAME` or `AWS_DEFAULT_*` as overrides.
- `findWorkspaceRoot` is removed from `quidproquo-deploy-awscdk`; the generic CDK app reads the workspace from cwd, which `qpq` always sets to the root. `quidproquo-config-aws` gains `getAwsServiceAccountInfosForDeployments(deploymentNames, moduleNames)`, which builds the cross-deployment rows for `defineAwsServiceAccountInfo` from deploy.config.json so account IDs are not repeated in service-utils.
- `quidproquo-config-aws` gains the deploy config reader (`getQpqAppDeployConfig`, `getQpqAppDeployment`, `validateQpqAppDeployConfig`, `primeDeploymentEnv`) and the aws narrowing (`QpqAwsPlatformSettings`, `QpqAwsAppDeployment`, `QpqAwsDeployEnvVar`, `validateAwsDeployment`, `primeAwsDeploymentEnv`); `quidproquo-core` gains the neutral types (`QpqAppDeployConfig`, `QpqAppDeployment`, `QpqAppDeploymentBase`, `QpqAppDeploySettings`, `QpqDeployEnvVar`). `QpqPlatformDriver.primeDeployIdentity(target): string[]` (quidproquo-cli) is replaced by `prepareDeployment(deploymentName, deployment): void`, which throws on an incomplete entry; `setupSteps` takes a deployment name instead of an environment.

## 0.1.27

- A KVS query or scan on a scoped store now refuses a `nextPageKey` that wasn't issued under the caller's scope (an edited key, or one replayed from another tenant), failing with `InvalidScope` (new `InvalidScopeErrorCode.pageKeyOutOfScope`) instead of reaching the store. Honest paging is unaffected. On the dev server the page key now records the scope that issued it, so a key issued before this change can't continue a scoped listing.
- The eventDoc list (`askEventDocListPage` and the collection list route, `quidproquo-features`) now reads newest first through the collection's `(type, updatedAt)` index instead of the table's id order. It names the index with the new `indexName` option on `askKeyValueStoreQuery` (`quidproquo-core`), which forces a declared index by its `name` and fails with the new `IndexNotFound` error type for an undeclared one. An index definition gains an optional `name`, defaulting to its partition key attribute (so existing indexes keep their deployed names); the resolved `KvsIndex` type now always carries `name`, so code building a `KvsIndex` by hand must set it. `defineKeyValueStore` throws when two indexes share a name, which is what two indexes on the same partition key without an explicit `name` now do. Without an `indexName` option, a query runs against the table if its keys cover every attribute in the key condition, else the first index whose keys do, so `type = X AND updatedAt > Y` now reaches a `(type, updatedAt)` index instead of failing against the table. A `nextPageKey` issued before the change can't continue a list after it (DynamoDB refuses a table cursor on an index query), so clients should restart paging. On the dev server, a query that resolves to a GSI now reads in that index's sort key order and skips rows missing its keys, as it does deployed.
- Scoped key value stores (`defineKeyValueStore` with `scoped: true`, `quidproquo-core`) now support GSIs. On AWS a GSI whose partition key isn't the table pk is keyed on a hidden `@@QPQGSI_<attribute>@@` attribute holding the scope-composed value; the index keeps its name and sort key. An existing scoped store that already declares such an index needs two deploys (remove the index and deploy, then re-add it and deploy), because CloudFormation can't change a GSI's key schema in place, and rows written before the new index exists only appear in it once rewritten. New rules on every backend: pk values can't contain `@@QPQ` (previously only the full `@@QPQSCOPE@@`; `KVS_RESERVED_MARKER` is now `@@QPQ`). On a scoped store, attribute names can't contain `@@QPQ`, a GSI partition key value must match its declared type, a GSI partition key can only be Set, SetIfNotExists or Removed as a whole (Increment, Add, Delete and nested paths are refused), and a key condition on a GSI partition key must use `=`; each fails with `InvalidScope` and one of the new `InvalidScopeErrorCode` members `reservedAttribute`, `indexKeyType` or `unsupportedOperation`. `defineKeyValueStore` throws for a scoped store with a binary index partition key.
- The DynamoDB scope composition helpers leave `quidproquo-core` and become internal to `quidproquo-actionprocessor-awslambda`: `KVS_SCOPE_DELIMITER`, `composeScopedKvsValue`, `decomposeScopedKvsValue`, `stripScopedKvsValue`, `stripScopedKvsItem`, `composeScopedKvsQueryOperation`, `composeScopedKvsQueryOperationOrThrow`, `validateScopedQueryConstrainsPkOrThrow`, `buildKvsScopeExclusionCondition`, `buildKvsScopeBeginsWithCondition`, `createScopedKvsTranslator`, `getScopedKvsTranslatorOrThrow` and the `ScopedKvsTranslator` type. Core keeps the backend-neutral scope rules: `resolveScopedKvsStoreOrThrow`, the existing `validateScopedKvs*OrThrow` checks, and the new `KVS_RESERVED_MARKER`, `validateKvsPkValueForScopeOrThrow`, `validateKvsKeyConditionForScopeOrThrow` and `validateKvsFilterForScopeOrThrow`. Stored data and runtime behaviour are unchanged.
...
- `defineStorageDrive` and `defineKeyValueStore` (`quidproquo-core`) gain a `scoped` option. When set, the drive/store now enforces the flag at call time: any file or KVS action against a `scoped` resource with no scope, or against an unscoped resource with a scope, throws `InvalidScopeError` (new codes `scopeRequired`/`notScoped`) instead of silently reading/writing the unpartitioned rows. `defineStorageDrive` also now throws at config time if `scoped` and `copyPath` are both set. Code relying on mixing scoped and unscoped calls against the same drive/store must either declare it `scoped` and always pass a scope, or drop the scope. `StorageDriveEvent` gains an optional `scope` field, populated when the drive is scoped.
- The `encryption: boolean` option on `defineStorageDrive` and `defineKeyValueStore` (`quidproquo-core`) is replaced by `cryptoKeyName?: string`, naming a `defineCryptoKey` to encrypt the resource with; the settings' `encryption` field is now `cryptoKeyName`. Every drive and store stays encrypted with the provider's managed keys when it is omitted. `defineAwsKmsKey` (`quidproquo-config-aws`) and `getAwsKmsKeys`/`getAwsKmsKeyForStorageDrive`/`getAwsKmsKeyForKeyValueStore` are removed: a key is provisioned by `defineCryptoKey` rather than bound by ARN. `QpqCoreStorageDriveConstruct` and `QpqCoreKeyValueStoreConstruct` (`quidproquo-deploy-awscdk`) take a resolved `cryptoKey` prop (see `resolveCryptoKeyForResource`) and throw at synth when the config names a key none was resolved for. A foreign key's owning service must be deployed before a service that encrypts with it.
- Tenant membership roles (`quidproquo-features`) move from a fixed `TenantMembershipRole` (`owner`/`member`) enum to an app-defined role catalog + direct permission grants. `TenantMembershipRole` is removed. `TenantMembership.role` is replaced by `roles: string[]` and a new `grants: TenantPermissionGrant[]` field, plus `rolesUpdatedAt`/`rolesUpdatedByUserId`. `TenantMember.role` becomes `roles: string[]`. `TenantMemberUpdateRequest.role` is removed (role assignment now has its own route); only `disabled` remains. `askTenantLinkMember` takes `roles: string[]` instead of a single `TenantMembershipRole`. `defineTenant` gains an optional `roles: { catalog?: TenantRoleCatalog; creatorRoles?: string[] }` option; the built-in `tenantAdmin` role (holding `tenant:members:manage` and `tenant:roles:assign`) replaces the old `owner` role and may not be redefined. `creatorRoles` (default `[tenantAdmin]`) sets what a tenant's creator is seeded with; every code must be in the catalog and at least one must hold `tenant:roles:assign`, checked at config time. `askTenantRolesConfigRead` now returns `{ catalog, creatorRoles }` instead of the bare catalog; code destructuring its result needs `const { catalog } = yield* askTenantRolesConfigRead()`. Code reading `membership.role`/`member.role` or building a `TenantMemberUpdateRequest` with `role` must migrate to the `roles` array and the new role-assignment route.
- `defineTenantedEventDoc` (`quidproquo-features`) collections now require a permission inside a tenant: every route checks `eventDoc:<storeName>:<action>` (`read`/`create`/`write`/`delete`) against the caller's tenant roles/grants, refusing with `Forbidden` if missing. Grant it via `defineTenant`'s `roles.catalog`, built from `eventDocPermissions(storeName)`. `createTenantedRouteDefinition`'s config gains an optional `permission` (`{ permission, resourceIdParam?, allowPersonalScope?, message? }`); a route with no `permission` stays membership-only, and a route with `permission` set now refuses a personal-scope (no tenant header) caller unless `allowPersonalScope` is set.
- The tenant collection (`quidproquo-features`) now lives under its own `TENANT#<id>` scope from creation instead of the creator's ambient scope, so a tenant is edited from inside itself (header + membership + `eventDoc:tenants:*` permission) rather than by whoever created it. `TenantRecord.scope` is removed; `getLogo` always presigns in the tenant's own scope, no fallback needed. Deployed tenants created before this change have their doc homed in the creator's old scope and must be migrated to `TENANT#<id>` to remain reachable.
- Tenant ids (`quidproquo-features`) are now the branded `TenantId` type instead of plain `string`, on `TenantRecord.tenantId`, `TenantMembership.tenantId`, `TenantResolvedRequest.tenantId` and every tenant story's `tenantId` parameter (`askTenantMembershipGet`, `askTenantMemberList`, `askTenantMemberAdd`, `askTenantMemberRemove`, `askTenantMemberUpdate`, `askTenantMemberRolesSet`, `askTenantValidateMembership`, `askTenantMembersWithPermission`, `askTenantLinkMember`, `askTenantRecordGet`, `askTenantLogoDownloadUrl`, `askTenantAssertCallerMayManageMembers`, and others). A plain `string` no longer type-checks where a `TenantId` is expected. Brand an id arriving on a request (header, path param, WebSocket claim) with `askTenantIdParse` (throws `BadRequest` on an invalid one); brand an id the system already minted or stored with `toTenantId`.
- `TenantCallerMembership` (`quidproquo-features`) gains a required `effectiveGrants: TenantPermissionGrant[]` field alongside the existing `permissions` list, returned by the `GET` membership route. Code constructing a `TenantCallerMembership` by hand must add it (e.g. via the new `tenantMembershipEffectiveGrants` helper).
- `defineEmailReceiver` (`quidproquo-webserver`) now returns a `QPQConfig` fragment instead of a bare `EmailReceiverQPQWebServerConfigSetting`, and owns its storage drive: `options.storageDriveName`/`options.keyPrefix` are replaced by a required `options.onEmail: QpqFunctionRuntime`, run once per message with an `EmailReceivedEvent` (`{ message: EmailMessage }`) instead of a raw drive object to parse yourself. Delete any `defineStorageDrive` declared for the receiver and move the handler's parsing logic into `onEmail`, reading `message` directly instead of calling `askEmailParse` on a `StorageDriveEvent`'s object. `EmailReceiverQPQWebServerConfigSetting.keyPrefix` is removed in favor of `onEmailFunctionName`.
...
- `getDomainName`, `getDomainRoot`, `resolveDomainRoot`, `getEnvironmentDomainName`, `getBaseDomainName`, `getServiceDomainName`, `constructServiceDomainName`, `constructEnvironmentDomainName`, `resolveApexDomainNameFromDomainConfig`, `getDnsConfigs` and `convertContentSecurityPolicyEntryToString` (`quidproquo-webserver`) are removed, as is the `askGetDomainRoot` story. Use `getRootDomains`, `resolveHosts(qpqConfig, target, resolver?)`, `resolvePrimaryHost`, `resolveHostForRoot`, `resolveOrigins`, `findRootDomainForHost` and the `askDnsResolveHosts` action instead. The hostname shape is a `DomainResolver` (`quidproquo-webserver`) that `defineDns` points at through a `QpqPureFunction` pointer (`{ basePath, relativePath, functionName }`, new in `quidproquo-core`; `QpqFunctionRuntimeAdvanced` now extends it). The tooling `require`s it (`requireDomainResolver` in `quidproquo-deploy-awscdk`), the runtime loads it through the dynamic module loader (`loadDomainResolver`; the pointer joins `getAllSrcEntries`), and `getCorsHeaders`, `getAllowedOrigins`, `buildOpenApiDocument` and `resolveHostForRequestHost` take the loaded resolver as a trailing argument.
- `defineDomainCertificate` (`quidproquo-config-aws`) is now `(region, targets: DomainTarget[], options?)`: one certificate per region covering every root, published under the app-keyed SSM parameter `/qpq/domain/certificate-arn/<region>/<app>-<env>[-<feature>]`. `getDomainCertificateArnSsmParameterName(region, qpqConfig)` and `lookupDomainCertificate(scope, certRegion, qpqConfig, idSuffix)` change accordingly. Existing per-root cert stacks (`<prefix>-cert-<root>`) become orphans: delete them by hand after every service has redeployed onto the new parameter. A host's Route53 zone is derived from the resolver output: the root's site root when the host sits under it, otherwise the root itself.
- `defineEmailSenderAllowList` (`quidproquo-config-aws`) takes only the address list; `getEmailSenderAllowedAddresses(qpqConfig)` drops its domain argument.
- `defineSubdomainRedirect`'s `addEnvironment` / `addFeatureEnvironment` flags no longer prefix the target: a bare `redirectUrl` that is a declared root redirects to that root's site root under the resolver, any other bare host is used literally, absolute urls redirect as-is. The redirect lambda now reads `redirectBaseUrl` / `appendPath` from its env instead of `redirectConfig`.
- `deploy.config.json` loses `domain` and `QpqAppDeployContext` loses `domain`: roots live only in `defineDns`, which the bootstrap fragment now declares itself (normally from the same constant the services use).
- `SubdomainName` (`quidproquo-deploy-awscdk`) takes `{ target, rootDomain }` instead of `{ subdomain, apexDomain, rootDomain }`; every domain-bearing construct now creates its resources on every root, the primary keeping today's logical ids and extra roots getting `-<root>` suffixed ids.
...
- `askEventDocValidateAppend` (`quidproquo-features`) now takes `(event, state)` instead of `(modelId, event)`; it no longer resolves document state itself, so callers must resolve and pass the state.
- `defineAwsGithubDeployRole` (`quidproquo-config-aws`) gains a `trustNameForm` option, defaulting to `false` once `ownerId`/`repositoryId` are given: the name-form OIDC subject (`repo:<owner>/<name>:...`) is no longer trusted alongside the immutable-id form unless explicitly requested. The deployed role's permissions also shrink from `AdministratorAccess` to only what a deploy needs (assume the CDK bootstrap roles, read/write this app's S3 buckets, read the deploy SSM parameters); a workflow that relied on the broader access must be re-scoped.
...
- `buildEventDocStore`'s options (`EventDocStoreOptions`) and the `EventDocStore` it returns drop `eventValidator`, `eventRenderer`, and `referenceResolver`.
- `EventDocTransferCollection` (`quidproquo-features`) drops `referenceResolver`. `defineEventDocTransfer`'s `collections` option now takes `EventDocTransferCollectionSource[]` instead of `EventDocTransferCollection[]` — each entry can be a live `EventDocFunctions` object, a collection-list entry carrying one under `functions`, or a bare `{ storeName, type, onPublish?, onAppend? }` registry entry.
